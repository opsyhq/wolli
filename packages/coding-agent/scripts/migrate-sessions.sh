#!/usr/bin/env bash
#
# Migrate sessions from ~/.steward/agent/*.jsonl to proper session directories.
# This fixes sessions created by the bug in v0.30.0 where sessions were
# saved to ~/.steward/agent/ instead of ~/.steward/agent/sessions/<encoded-cwd>/.
#
...
AGENT_DIR="${STEWARD_AGENT_DIR:-$HOME/.steward/agent}"
DRY_RUN=false
