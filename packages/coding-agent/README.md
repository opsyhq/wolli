...
- **Update check:** fetches `https://pi.dev/api/latest-version` to check whether a newer Pi version exists. Disable it with `STEWARD_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
- **Install/update telemetry:** after first install or a changelog-detected update, sends an anonymous version ping to `https://pi.dev/api/report-install`. This setting also controls optional provider attribution headers for OpenRouter, Cloudflare, and direct NVIDIA NIM requests. Opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `STEWARD_TELEMETRY=0`. This does not disable update checks; Pi may still contact `pi.dev` for the latest version unless update checks are disabled or offline mode is enabled.

Use `--offline` or `STEWARD_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
|----------|-------------|
| `STEWARD_CODING_AGENT_DIR` | Override config directory (default: `~/.steward/agent`) |
| `STEWARD_CODING_AGENT_SESSION_DIR` | Override session storage directory (overridden by `--session-dir`) |
| `STEWARD_PACKAGE_DIR` | Override package directory (useful for Nix/Guix where store paths tokenize poorly) |
...
| `STEWARD_TELEMETRY` | Override install/update telemetry and provider attribution headers. Use `1`/`true`/`yes` to enable or `0`/`false`/`no` to disable. This does not disable update checks |
| `STEWARD_CACHE_RETENTION` | Set to `long` for extended prompt cache (Anthropic: 1h, OpenAI: 24h) |
