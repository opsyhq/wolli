...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.STEWARD_TELEMETRY;
		delete process.env.STEWARD_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.STEWARD_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/opsyhq/steward");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe(APP_NAME);
	});
