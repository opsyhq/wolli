...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.STEWARD_OFFLINE);
	if (offlineMode) {
		process.env.STEWARD_OFFLINE = "1";
		process.env.STEWARD_SKIP_VERSION_CHECK = "1";
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.STEWARD_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: STEWARD_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
