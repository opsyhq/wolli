...
Steward is a persistent, purposeful agent that runs in your terminal. Unlike a
chat session, a Steward agent is created *for* something — a purpose stated by
...
Steward reuses credentials that another agent CLI may have already set up under
`~/.steward/agent/` (API keys and OAuth tokens). To set a key explicitly, export
it before starting:
...
```
steward new <name> [--model provider/id]      Create an agent, then start its birth conversation
steward list                                  List agents
steward delete <name>                          Delete an agent (type-the-name confirm)
steward <name> [message] [--new] [--print]    Talk to an agent
```
...
Running `steward <name>` drops you into an interactive terminal UI (TUI). By
...
```bash
steward calories "log: two eggs and toast" --print
```
...
[Agent homes](#agent-homes)). When you start an agent without `--new`, Steward
...
Steward keeps each agent's state in its own home directory so deleting an agent
also cleans up everything attached to it.

| Path | Contents |
|------|----------|
| `~/.steward/agents/<name>/` | Per-agent home |
| `~/.steward/agents/<name>/agent.json` | Identity config (name, purpose, deploy state) |
| `~/.steward/agents/<name>/SOUL.md` | Curated identity |
| `~/.steward/agents/<name>/MEMORY.md` | Curated durable memory |
| `~/.steward/agents/<name>/USER.md` | Curated facts about the human |
| `~/.steward/agents/<name>/sessions/` | Append-only session tree |
| `~/.steward/agents/<name>/workspace/` | The agent's owned working directory |
| `~/.steward/agents/<name>/{extensions,skills,prompts,themes}/` | Per-agent customizations |
| `~/.steward/agent/` | Shared credentials (`auth.json`) and default model settings |

Override the root with `STEWARD_HOME` (defaults to `~/.steward`).
