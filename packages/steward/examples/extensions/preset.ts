 ...
 * - ~/.steward/agents/<name>/presets.json (global)
 * - <cwd>/.steward/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `steward --preset plan` - start with plan preset
...
export default function presetExtension(steward: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: steward.getThinkingLevel(),
				tools: steward.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			steward.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = steward.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				steward.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		// Note: ui.custom() is a no-op in Steward; the rest of this example works.
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await steward.setModel(originalState.model);
				}
				steward.setThinkingLevel(originalState.thinkingLevel);
				steward.setActiveTools(originalState.tools);
			} else {
				steward.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	steward.registerShortcut(Key.ctrlShift("u"), {
	...
	steward.registerCommand("preset", {
	...
	steward.on("before_agent_start", async (event) => {
	...
	steward.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = steward.getFlag("preset");
	...
	steward.on("turn_start", async () => {
		if (activePresetName) {
			steward.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
