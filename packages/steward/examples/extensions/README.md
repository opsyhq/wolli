...
Example extensions for Steward.

## Usage

```bash
# Load an extension with --extension flag
steward --extension examples/extensions/permission-gate.ts

# Or copy to the agent's extensions directory for auto-discovery
cp permission-gate.ts ~/.steward/agents/<name>/extensions/
```
...
| `plan-mode/` | Plan mode for read-only exploration with `/plan` command and step tracking |
...
| `space-invaders.ts` | Space Invaders game with custom UI and keyboard handling |
...
| `send-user-message.ts` | Demonstrates `steward.sendUserMessage()` for sending user messages from extensions |
...
| `summarize.ts` | Summarize conversation and show in transient UI |
...
| `git-merge-and-resolve.ts` | Walks git merge conflicts and resolves them interactively |
...
| `system-prompt-header.ts` | Prepends a header to the system prompt |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs Steward theme with macOS dark/light mode |
| `bash-spawn-hook.ts` | Runs a hook when the agent spawns a bash subprocess |
...
| `event-bus.ts` | Inter-extension communication via `steward.events` |
| `send-user-message.ts` | Sends user messages from an extension via `steward.sendUserMessage()` |
...
| `commands.ts` | Registers slash commands and shortcuts |
| `border-status-editor.ts` | Shows status in the editor border |
...
| `prompt-customizer.ts` | Customizes the system prompt at startup |
| `working-message-test.ts` | Demonstrates the transient working message |

### Provider Inspection

| Extension | Description |
|-----------|-------------|
| `provider-payload.ts` | Logs the LLM request payload and response status via `before_provider_request` / `after_provider_response` |
