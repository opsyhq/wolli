import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "@opsyhq/steward";
...
export default function (steward: ExtensionAPI) {
	...
	steward.on("agent_end", () => {
		isWorking = false;
		stopSpinner();
		activeTui?.requestRender();
	});

	steward.on("session_shutdown", () => {
		stopSpinner();
		activeTui = undefined;
	});
	...
	steward.on("session_start", (_event, ctx) => {
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = steward.getThinkingLevel();
