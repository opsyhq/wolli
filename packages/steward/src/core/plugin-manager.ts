...
function isOfflineModeEnabled(): boolean {
	const value = process.env.STEWARD_OFFLINE;
...
// Steward is per-agent only: the sole non-temporary install scope is "user"
...
type SkillDiscoveryMode = "steward";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "steward" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "steward");
	}
...
export class DefaultPluginManager implements PluginManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension/integration packages run inside steward and resolve host APIs through loader
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "steward-extensions", private: true };
	...
	private addAutoDiscoveredResources(
		...
		// dirs. There is no project-local (`.steward/`) or ancestor `.agents/`
		...
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "steward"),
			userMetadata,
