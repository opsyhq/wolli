...
> **Note:** The integration factory's first argument is named `steward` throughout this document — by convention, the same as the extension factory argument. It is the `IntegrationsAPI` object; call it whatever you like. The package.json manifest key that declares integrations is `"steward"`; that key name is fixed and unrelated to the argument name.
>
> An integration has two halves. The **producer/transport** half (`run`, `actions`, `events`, `onboard`) is the integration module itself, registered via `steward.registerIntegration`. The **mapping** half is an ordinary extension that consumes the integration via `steward.getIntegration(...)`. The two ship together as one package (see [The dual-half package](#the-dual-half-package)).
...
export default function (steward: ExtensionAPI) {
  const ticker = steward.getIntegration("ticker", "default");
...
```bash
steward <agent> plugins install ./path/to/ticker
```
...
| Location | Scope |
|----------|-------|
| `~/.steward/agents/<name>/integrations/*.ts` | The agent (all sessions) |
| `~/.steward/agents/<name>/integrations/*/index.ts` | The agent (subdirectory) |

Plugins installed with `steward <agent> plugins install <source>` are resolved into this folder by the package manager. Configured account credentials live separately, in the per-agent `integrations.json`:
...
| Package | Purpose |
|---------|---------|
| `@opsyhq/steward` | Integration types (`IntegrationsAPI`, `IntegrationConfig`, `IntegrationOnboardContext`, `IntegrationRunContext`, `IntegrationActionContext`, `KeyValueStore`, `IntegrationHandle`) |
| `typebox` | Schemas (`Type`) for `account`, `events`, and action `parameters` |

The integration types are host-provided. A shipped integration package declares `@opsyhq/steward` as a **peerDependency**, not a dependency — the host supplies it at load time. The package brings its own transport dependencies (the Telegram plugin bundles `grammy` + `@grammyjs/runner`; the scheduler bundles `croner`).
...
An integration module default-exports a factory receiving `IntegrationsAPI`. Inside, call `steward.registerIntegration(config)` once per service. The factory may be synchronous or asynchronous.
...
`run` is the one genuinely new concept in steward, which is otherwise pull/request-response. It opens a connection or loop and pushes inbound items out as events. The host calls `run` once per `(service, account)` after the account validates.
...
`ctx.store` is a string-keyed `KeyValueStore` for machine-written runtime state, scoped to one service and backed by `~/.steward/agents/<name>/store/<service>.json`. It is process-scoped and survives `/reload`. Use it where an integration keeps state it owns (the scheduler keeps its jobs here).
...
The mapping half is an ordinary extension. It reaches the transport through `steward.getIntegration(name, account?)`, which returns an `IntegrationHandle`. `account` defaults to `"default"`.
...
export default function (steward: ExtensionAPI) {
  const tg = steward.getIntegration("telegram", "default"); // throws if telegram.default not configured
...
A shipped integration is one package declaring both halves under its package.json `"steward"` key:
...
Why the split is load-bearing: the transport (`index.ts`) never touches sessions or the agent; the extension (`*-chat.ts`) owns the mapping between the external thread and a Steward session. Swapping the mapping is an extension change; the transport is untouched.
...
```bash
steward <agent> plugins install <source>      # install + auto-onboard (on a TTY)
steward <agent> plugins configure <source>    # re-run guided setup (requires a TTY)
steward <agent> plugins list                  # list installed plugins + contributed integrations
steward <agent> plugins remove <source>       # remove the plugin
steward <agent> plugins update [source]       # update one or all plugins
```
...
| Form | Example |
|------|---------|
| local | `steward <agent> plugins install ./plugins/telegram` |
| npm | `steward <agent> plugins install npm:@scope/pkg` |
| git | `steward <agent> plugins install git:github.com/user/repo` |
...
The Telegram plugin is the canonical dual-half integration. The transport (`index.ts`) long-polls grammY, holds the bot token, and emits a `message` event per inbound message. It exposes `sendMessage`, `sendChatAction`, and `setCommands` actions. The mapping (`telegram-chat.ts`) routes each message into a per-chat Steward session and ships the reply back.
