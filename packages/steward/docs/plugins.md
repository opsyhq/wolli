...
- [The package.json `steward` Manifest](#the-packagejson-steward-manifest)
...
A plugin is a directory (or published package) with a `package.json` whose `"steward"` field names the contribution files. Each listed path is a normal source module loaded by the agent's resource loader at launch:

- **integrations** — transport modules registered via `steward.registerIntegration` (see [integrations.md](./integrations.md)).
...
The agent's discovery dirs (`~/.steward/agents/<name>/extensions/`, `integrations/`, `skills/`, etc.) are for hand-placed local resources. Installed plugins are **not** unpacked into those dirs — they stay in `.plugins/` and are resolved from there.

## The package.json `steward` Manifest

The plugin manager reads `package.json` and parses exactly the `"steward"` object. Every key is an array of paths (relative to the package root) or glob patterns:
...
No other keys under `"steward"` are read. (There is no gallery/preview metadata; steward has no package registry of its own — see [Publishing](#publishing).)
...
```json
{
  "name": "steward-integration-telegram",
  "type": "module",
  "steward": {
...
- If no `"steward"` manifest is present, the manager falls back to convention directories — `extensions/`, `integrations/`, `skills/`, `prompts/`, `themes/` — and auto-discovers files there. A bare file or a manifest-less directory with no convention dirs is treated as a single extension.
...
### Why peerDependencies on `@opsyhq/steward`

Contribution modules import host types and APIs from `@opsyhq/steward` (`IntegrationsAPI`, `ExtensionFactory`, etc.). The host process *provides* that package at runtime, so the plugin must not bundle its own copy. Declare it as a peer with a `"*"` range:

```json
{ "peerDependencies": { "@opsyhq/steward": "*" } }
```
...
```
my-plugin/
├── package.json        # name, type: "module", "steward" manifest, deps, peerDependencies
...
2. **Declare them in the manifest.** List each file under the matching `"steward"` key (above).
3. **Set `"type": "module"`** so `.ts`/`.js` modules load as ESM.
4. **Put runtime deps in `dependencies`** and **`@opsyhq/steward` in `peerDependencies`** with `"*"`.
...
A plugin is shared as an ordinary package. Steward installs from three source kinds and nothing else — there is no steward-hosted registry or installer. Pick the distribution that matches the source scheme you want users to install with:

| Distribution     | How users install                                         |
|------------------|-----------------------------------------------------------|
| npm registry     | `npm publish`, then `steward <agent> plugins install npm:<name>` |
...
Steward does not run `npm publish` for you; it only *consumes* one of these three source forms.
...
```bash
steward <agent> plugins install npm:@scope/pkg              # npm: registry package
steward <agent> plugins install npm:@scope/pkg@1.2.3        # pinned version
steward <agent> plugins install git:github.com/user/repo    # git: shorthand
steward <agent> plugins install git:github.com/user/repo@v1 # pinned ref
steward <agent> plugins install ./path/to/plugin            # local path
```
...
```bash
steward <agent> plugins install ./packages/steward/plugins/telegram   # installs, then auto-onboards if on a TTY
steward <agent> plugins configure ./packages/steward/plugins/telegram # re-run guided setup on demand
```

- **On install, on an interactive terminal (TTY):** onboarding runs immediately. Each onboarded service prints one result: `connected` (then a hint to run `steward <agent>` to use it), `cancelled` (you dismissed a prompt), `not-found` (the integration is not installed for the agent), `no-onboard` (it declares no guided setup), or an `error` with its message. Any non-`connected`/`cancelled` result makes the command exit non-zero.
- **On install, non-interactive (no TTY):** install completes but skips setup and points you at `steward <agent> plugins configure <source>` to finish later.
...
{
  "plugins": [
    "npm:steward-integration-scheduler",
    {
...
The shipped Telegram plugin (`packages/steward/plugins/telegram/`) is the canonical dual-half plugin. Its layout:

```
telegram/
├── package.json        # "steward": { integrations: ["./index.ts"], extensions: ["./telegram-chat.ts"] }
...
└── telegram-chat.ts    # mapping extension: routes each message into a per-chat Steward session,
...
```bash
steward my-agent plugins install ./packages/steward/plugins/telegram
# -> copies the package into ~/.steward/agents/my-agent/.plugins/local/telegram-<hash>/
# -> records "plugins": ["packages/steward/plugins/telegram"] in agent.json (agent-relative)
# -> runs onboard: prompts for the bot token, writes the account to integrations.json
```

Both `index.ts` and `telegram-chat.ts` resolve from that one copy. The transport starts; once the account is configured, the mapping extension activates and bidirectional chat is live. To package it for others, publish the same directory to npm (`steward-integration-telegram`) or a git repo and have them install with `npm:` / `git:` instead of the local path.

The shipped scheduler plugin (`packages/steward/plugins/scheduler/`) has the identical shape — `"integrations": ["./index.ts"]`, `"extensions": ["./scheduler-chat.ts"]`, one runtime dep (`croner`), and the same `@opsyhq/steward` peer — confirming the dual-half pattern is the convention, not Telegram-specific. The only manifest differences are the package name, the dependency, and the extension filename.
