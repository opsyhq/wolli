...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@opsyhq/wolli` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@opsyhq/wolli";
	 *
...
/** Options accepted by `wolli.registerCommand`. */
...
export interface ExtensionAPI {
	 ...
	 * `wolli.environments.targets[...]` to reach a specific target.
	 */
	...
	/** Gracefully shut down wolli and exit. */
	shutdown(): void;
	 ...
	 * wolli.registerProvider("my-proxy", {
	 ...
	 * wolli.registerProvider("anthropic", {
	 *   baseUrl: "https://proxy.example.com"
	 * });
	 *
	 * @example
	 * // Register provider with OAuth support
	 * wolli.registerProvider("corporate-ai", {
	 ...
	 *
	 * @example
	 * wolli.unregisterProvider("my-proxy");
	 */
...
/** Configuration for registering a provider via wolli.registerProvider(). */
...
export type ExtensionFactory = (wolli: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	// Agent-global capabilities backing the `wolli.*` methods. Throwing stubs during extension load;
	// the runtime overrides them (closures over the AgentRuntime) once resources are built — so accessing
	// them during load throws (mirrors the old `wolli.agent` resolution).
	getSession: (id: string) => Session | undefined;
	openSession: (id: string) => Promise<Session>;
	createSession: (options?: NewSessionOptions) => Promise<Session>;
	listSessions: () => Promise<SessionInfo[]>;
