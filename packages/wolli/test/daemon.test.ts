...
/** Write a minimal self-contained local package fixture (a `wolli` manifest + its files). */
function writePackage(dir: string, wolli: Record<string, string[]>, files: Record<string, string>): string {
	mkdirSync(dir, { recursive: true });
	writeFileSync(join(dir, "package.json"), JSON.stringify({ name: basename(dir), wolli }));
...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "wolli-daemon-home-"));
...
afterEach(async () => {
	...
	delete process.env.WOLLI_SERVICE_MANAGER;
