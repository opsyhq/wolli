...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "wolli-svc-"));
...
describe("detectServiceManager", () => {
	it("honors the WOLLI_SERVICE_MANAGER override", () => {
...
describe("buildLaunchAgentPlist", () => {
	const plist = buildLaunchAgentPlist({
		...
		environment: { WOLLI_HOME: "/home/.wolli", AMP: "a&b" },
	});
	...
	it("renders the program arguments and environment, XML-escaped", () => {
		...
		expect(plist).toContain("<key>WOLLI_HOME</key>");
		expect(plist).toContain("<string>/home/.wolli</string>");
...
describe("buildSystemdUnit", () => {
	const unit = buildSystemdUnit({
		description: "wolli agent test",
		execStart: ["/usr/bin/node", "/opt/my cli.js", "daemon", AGENT],
		workingDirectory: "/home/agent",
		environment: { WOLLI_HOME: "/home/.wolli" },
	});

	it("renders an always-on user service", () => {
		expect(unit).toContain("Restart=always");
		expect(unit).toContain("WantedBy=default.target");
		expect(unit).toContain("Environment=WOLLI_HOME=/home/.wolli");
	});
	...
	it("names the unit per agent", () => {
		expect(systemdUnitName(AGENT)).toBe(`wolli-${AGENT}.service`);
	});
});
