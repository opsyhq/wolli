...
// Registers a `/cap` command whose handler closes over the agent-global `wolli`; calling a wolli
...
export default function staleExtension(pi) {
	pi.registerCommand("cap", {
		description: "Calls a wolli method; throws once the runner is superseded.",
		async handler() {
...
// stored-session count (off `wolli.listSessions()`) to the marker dir.
...
export default function facadeExtension(pi) {
	pi.registerCommand("facade", {
		description: "Records the acting session id + session count via ctx.session/wolli.listSessions().",
...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "wolli-ext-home-"));
	sharedDir = mkdtempSync(join(tmpdir(), "wolli-ext-shared-"));
	markerDir = mkdtempSync(join(tmpdir(), "wolli-ext-marker-"));
...
describe("extension subsystem wiring", () => {
	...
	it("invalidates a captured wolli on reload", async () => {
		...
		// The captured wolli works before the reload.
		...
		// Any wolli captured from the superseded runner now throws stale.
	...
	it("exposes ctx.session/wolli.listSessions() so a command resolves the acting session and lists sessions", async () => {
		...
		// wolli.listSessions() surfaced the stored session create() wrote eagerly.
