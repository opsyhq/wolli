...
/** Write a minimal self-contained local package fixture (a `voli` manifest + its files). */
function writePackage(dir: string, voli: Record<string, string[]>, files: Record<string, string>): string {
	mkdirSync(dir, { recursive: true });
	writeFileSync(join(dir, "package.json"), JSON.stringify({ name: basename(dir), voli }));
...
beforeEach(() => {
	home = mkdtempSync(join(tmpdir(), "voli-daemon-home-"));
	sharedDir = mkdtempSync(join(tmpdir(), "voli-daemon-shared-"));
	process.env.VOLI_HOME = home;
	process.env.VOLI_SHARED_DIR = sharedDir;
...
afterEach(async () => {
	...
	delete process.env.VOLI_HOME;
	delete process.env.VOLI_SHARED_DIR;
	delete process.env.VOLI_SERVICE_MANAGER;
	delete process.env.VOLI_OFFLINE;
