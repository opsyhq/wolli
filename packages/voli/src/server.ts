...
async function createAgentRuntime(name: string): Promise<AgentRuntime> {
	...
	// (`~/.voli/agents/<name>/auth.json`) defers to the global tier on a per-provider miss.
	const globalAuth = AuthStorage.create();
	const authStorage = AuthStorage.create(getAgentAuthPath(name), globalAuth);
	// Integration accounts are per-agent (`~/.voli/agents/<name>/integrations.json`).
	const integrationAccounts = IntegrationAccountStorage.create(name);
	// Integration runtime state is per-agent, one file per service (`~/.voli/agents/<name>/store/`).
 ...
 * `VOLI_DAEMON_HOST` and `--port` override) and blocks on the listening server until a signal — or a
...
export async function runDaemon(name: string, opts: RunDaemonOptions = {}): Promise<number> {
	...
	// Fixed per-agent port + token from agent.json; `--port` and VOLI_DAEMON_TOKEN override.
