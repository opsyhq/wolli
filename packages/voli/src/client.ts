/**
 * The `@opsyhq/voli` client surface:
 *   - `Voli`       — the agent collection (`list`/`get`/`create`).
...
export class Agent {
	...
	async connect(): Promise<void> {
		...
		if (!(await isHealthy(base))) {
			// The launch command goes through the running binary, since `voli` isn't on PATH in dev.
