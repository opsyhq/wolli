...
// File tools live under tools/. memory is voli's own curated-notes tool; the
...
/**
 * Map voli's loaded skills/prompt templates into the shapes the harness exposes
 * for explicit invocation (`harness.skill()` / `harness.promptFromTemplate()`).
 *
 * Only skills need converting: voli's `Skill` carries just a `filePath` (the body
 * is never held in memory), so a skill's `content` is the frontmatter-stripped
 * SKILL.md body, read from disk here. Voli's `PromptTemplate` already carries
...
export class AgentRuntime {
	...
	/**
	 * The shared run-target map exposed to extensions via `voli.environments`. Built once in
	...
	/**
	 * The shared run-target map (backs `voli.environments`). Throws before the first
	...
	/** Install the graceful-shutdown handler exposed to extensions via `voli.shutdown()`. */
	setShutdownHandler(handler: () => void): void {
		this._shutdownHandler = handler;
	}

	/** Fire the graceful-shutdown handler (backs `voli.shutdown()`). */
	shutdown(): void {
	 ...
	 * every `reload()`. Mutates the shared runtime state and wires the `voli.*` runtime delegates.
	 */
	private async buildSharedResources(): Promise<void> {
		...
		// host-escalation dialogs raised through `voli.environments` to a live session's UI (best-
		...
		// Wire the agent-global `voli.*` delegates onto the shared extension runtime. Closures over
		...
		// A post-bind `voli.registerTool()` refreshes every resident session's tools immediately (the
	...
	private buildSessionTooling(session: AgentSession): SessionTooling {
		...
		// Tools operate in the agent's home dir. memory is voli's curated-notes tool; the rest are
...
export class AgentSession {
	...
	async prompt(text: string, options?: ConversationPromptOptions): Promise<void> {
		...
		// 1. Extension command — manages its own LLM interaction via voli.sendMessage().
	 ...
	 * returned from `voli.getSession()`. The object is a curated facade over this session: closures/
